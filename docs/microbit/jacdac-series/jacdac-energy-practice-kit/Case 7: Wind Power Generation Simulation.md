...
倾斜micro:bit主板来控制"风力"的大小——**倾斜角度越大，代表风力越强**，360°舵机驱动风力发电机叶片旋转的转速也越快；将主板放平时（无风状态），叶片停止转动。通过加速度计感知倾斜角度并实时调节转速，模拟风力发电机随风运转的真实场景。
...
![风力发电机](placeholder-wind-turbine.png)
...
| 触发条件 | 动作 | 来源 |
|---|---|---|
| 主板向前倾斜（pitch角变化） | 根据倾斜角度划分风力等级，控制舵机以对应转速旋转 | micro:bit V2 加速度计 |
| 主板放平（pitch角≈0°） | 舵机停止旋转 | micro:bit V2 加速度计 |
...
也许你并没有意识到，但你每天都在使用加速度计：

| 应用场景 | 加速度计的作用 |
|---|---|
| 手机横竖屏自动切换 | 检测手机倾斜方向 |
| 计步器/运动手环 | 通过分析行走时的加速度波形来统计步数 |
| 汽车安全气囊弹出 | 检测剧烈碰撞时的瞬间减速度 |
| 地震监测 | 检测地面的微小震动和加速度变化 |
| 游戏手柄（体感操作） | 感知手柄的倾斜和挥动动作 |
| 无人机/航拍器 | 保持机身平衡和姿态稳定 |
| 笔记本电脑硬盘保护 | 检测到自由落体时立即锁死磁头防损坏 |

#### 加速度计 vs 陀螺仪：两种传感器，各有分工

| 对比维度 | 加速度计（Accelerometer） | 陀螺仪（Gyroscope） |
|---|---|---|
| 测量什么 | 线加速度 + 相对于重力的倾斜角度 | 绕轴旋转的角速度（°/秒） |
| 擅长 | 静态倾斜检测、振动检测 | 快速旋转追踪、姿态变化 |
| micro:bit是否有 | ✅ 有（内置） | ❌ 没有（需要外接模块） |
| 典型应用 | 横竖屏切换、计步器、水平仪 | 游戏手柄体感、VR头盔追踪、无人机飞行控制 |
| 打个比方 | 就像你闭眼站着，靠重力感知自己是否站歪了 | 就像你快速转圈时，能感觉到自己正在旋转 |

> 💡 实际上，手机和平板电脑通常**同时内置**了加速度计和陀螺仪两种传感器，配合使用可以实现更精准的运动追踪。micro:bit上虽然没有陀螺仪，但利用加速度计已经可以做出很多有趣的项目——比如你用倾斜控制的风力发电机！
