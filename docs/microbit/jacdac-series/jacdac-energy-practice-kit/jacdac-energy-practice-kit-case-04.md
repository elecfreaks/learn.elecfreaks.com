...
本案例使用micro:bit主板内置的**麦克风**作为传感器，实时采集环境声音响度（0 ~ 255），将其同时映射为**LED灯环亮度**（0 ~ 100）和**舵机转速**（0 ~ 100），实现声控联动——声音越大、灯越亮、浮子起伏越剧烈。
...
如下图所示，将micro:bit主板插入Jacdac扩展板，用连接线舵机模块采用**菊花链方式**连接在Jacdac扩展板金手指接口上。
...
| 触发条件 | 动作 | 来源 |
|---|---|---|
| 按下按键A | 360°积木舵机以设定速度持续旋转，驱动凸轮带动浮子往复运动，模拟波浪发电 | micro:bit V2 按键A |
| 按下按键B | 360°积木舵机停止转动，模拟电站停机 | micro:bit V2 按键B |
