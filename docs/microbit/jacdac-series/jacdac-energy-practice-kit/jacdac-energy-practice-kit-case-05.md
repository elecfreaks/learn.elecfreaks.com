...
### 电子罗盘（磁力计）——micro:bit的"方向感知器"

micro:bit主板内置了一颗**MEMS磁力计**芯片，可以检测地球磁场在X、Y、Z三个轴上的分量，从而计算出主板相对于地磁北极的方位角。

>  **注意：** 磁力计容易受到周围金属物体和电磁设备的干扰。使用前建议先进行**校准**——在MakeCode编辑器中旋转micro:bit主板完成"八字校准"，以获得准确的方位读数。
...
如下图所示，将micro:bit主板插入Jacdac扩展板，用连接线将LED模块采用**菊花链方式**连接在Jacdac扩展板金手指接口上。
...
| 触发条件 | 动作 | 来源 |
|---|---|---|
| 开机后持续运行 | 读取磁力计方位角（0°~359°），显示、东、西、南、北 | micro:bit V2 磁力计 |
