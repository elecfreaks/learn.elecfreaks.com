---
sidebar_position: 1
sidebar_label: Product Introduction
---

# Jacdac Smart Exploration Kit

---
## Introduction
The Jacdac Smart Exploration Kit, which is based on micro:bit V2, is equipped with a variety of Jacdac series sensors, including jacdac expansion boards, button sensors, light sensors, human infrared sensors, magnetic sensors, knob encoders, light ring modules, etc. Jacdac supports plug-and-play functions, can automatically identify sensor types, and provide real-time feedback of sensor values in the simulation area of programming MakeCode, thereby helping users quickly build smart creative projects. In addition, the kit contains 10 basic cases, designed to help beginners get started quickly and stimulate their interest in programming and electronic components.

![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/hardware-connection-diagram/jacdac-smart-exploration-kit-01.png)

---
## Product List

|Item|Diagram|Quantity|
|:-----:|:-----:|:-----:|
|jacdac expansion board|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20bit.png)|1|
|button sensors|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Button%20sensor.png)|2|
|knob encoders|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Rotray%20Encoder%20sensor.png)|1|
|human infrared sensors|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Motion%20Sensor.png)|1|
|Light sensor|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Light%20Sensor.png)|1|
|Magnetic sensor|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Magnet%20Sensor.png)|1|
|Light Ring|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20LED%20Ring.png)|1|
|Jacdac-10cm Connection lines|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/Jacdac-smart-exploration-kit-10cm-cable.png)|5|
|Jacdac-25cm Connection lines|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/Jacdac-smart-exploration-kit-25cm-cable.png)|1|
|USB Cable|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/usb%20cable1.png)|1|

---
## Product Features

1. Supports plug-and-play function, allowing modules to be added or removed at any time during operation without restarting the main control device. The system can automatically identify the sensor type and provide real-time feedback on sensor information.
2. Jacda's various sensors, actuators, and other electronic modules, through unified software and hardware design standards, can be flexibly combined to help users quickly build projects.
3. Compatible with building block expansion design to achieve free assembly.
4. Provides 10 interesting entry-level cases to help users get started easily and quickly.

---
## Sensor Introduction

|Picture|Name|Introduction|
|:-----:|:-----:|:-----:|
|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20bit.png)|jacdac expansion board|Used to connect to the micro:bit board|
|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Button%20sensor.png)|button sensors|A sensor used to detect the state of a button, outputting a signal carrying "0" and "1".|
|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Rotray%20Encoder%20sensor.png)|knob encoders|Sensors for detecting the direction and value of rotational movements.|
|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Motion%20Sensor.png)|human infrared sensors|Sensors that sense human movement.|
|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Light%20Sensor.png)|Light sensor|Sensor for detecting ambient light intensity.|
|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20Magnet%20Sensor.png)|Magnetic sensor|Sensors used to measure the strength and direction of magnetic fields.|
|![](https://wiki-media-ef.oss-cn-hongkong.aliyuncs.com/docs/microbit/getting-started/microbit-jacdac-smartexploration-kit/images/sensor/jacdac%20LED%20Ring.png)|Light Ring|A light ring composed of 8 RGB lights.|

